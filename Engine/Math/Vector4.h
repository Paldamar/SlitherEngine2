...
public:
	...
	inline Vector4 GetNormalized() const { float len = Length(); if (MathLibrary::fequal(len, 0)) return Vector4(x, y, z, w); len = 1.0f / len; return Vector4(x * len, y * len, z * len, w * len); }
	inline Vector4 Normalize() { float len = Length(); if (!MathLibrary::fequal(len, 0)) { x /= len; y /= len; z /= len; w /= len; } return *this; }
