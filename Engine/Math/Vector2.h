...
public:
	...
	inline Vector2 GetNormalized() const { float len = Length(); if (MathLibrary::fequal(len, 0)) return Vector2(x, y); len = 1.0f / len; return Vector2(x * len, y * len); }
	inline Vector2 Normalize() { float len = Length(); if (!MathLibrary::fequal(len, 0)) { x /= len; y /= len; } return *this; }
	...
	inline bool operator ==(const Vector2& o) const { return MathLibrary::fequal(this->x, o.x) && MathLibrary::fequal(this->y, o.y); }
	inline bool operator !=(const Vector2& o) const { return !MathLibrary::fequal(this->x, o.x) || !MathLibrary::fequal(this->y, o.y); }
